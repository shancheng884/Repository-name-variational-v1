...
def _runtime(tmp_path) -> VariationalToLighterRuntime:
    ...
    runtime.live_inventory_state_file = tmp_path / "live_inventory_state.json"
...
def test_live_inventory_state_open_blocks_startup(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("LIGHTER_ACCOUNT_INDEX", "1")
    monkeypatch.setenv("LIGHTER_API_KEY_INDEX", "1")
    monkeypatch.setenv("LIGHTER_PRIVATE_KEY", "secret")
    runtime = _runtime(tmp_path)
    ...
    runtime.write_live_inventory_state(
        {
            ...
            "asset": "BTC",
            "next_lot_id": 2,
            "open_lots": [{"lot_id": 1, "direction": "long_var_short_lighter", "qty": "0.0001"}],
        }
    )
